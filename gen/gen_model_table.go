...
func ModelTableName(protoFileName string) {
	// step 0: 判断文件是否存在
	...
	proto.Walk(definition,
		...
		proto.WithMessage(func(message *proto.Message) {
			...
			if strings.HasPrefix(message.Name, MessagePrefixModel) {
				tableNameMap[message.Name] = &temp.Table{
					TableName:   genTableName(protoFileName, message.Name),
	...
	lines.WriteString("\n")
	lines.WriteString(fmt.Sprintf("package %s", TrimProtoFileNameSuffix(protoFileName)))
	lines.WriteString("\n")
